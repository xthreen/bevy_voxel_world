...
fn update_cursor_cube(
    ...
    for ev in cursor_evr.read() {
        ...
        let (camera, cam_gtf) = camera_info.single().expect("Query should find camera and global transform.");
        ...
        if let Some(result) = voxel_world_raycast.raycast(ray, &|(_pos, _vox)| true) {
            let (mut transform, mut cursor_cube) = cursor_cube.single_mut().expect("Query should find a transform and cursor cube.");
...
fn mouse_button_input(
    ...
    if buttons.just_pressed(MouseButton::Left) {
        let vox_pos = cursor_cube.single().expect("Query should find the cursor cube.").voxel_pos;
